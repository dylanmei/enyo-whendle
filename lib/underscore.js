...
_.mixin({
  ...
  isLocation: function(v) {
    return (v && (v.latitude || v.latitude == 0) && (v.longitude || v.longitude == 0));
  }
});
...
(function() {
  // FIND A HOME FOR FORMATTING
  var HH24 = _.template('#{hour}:#{minute}');
  var HH12 = _.template('#{hour}:#{minute} #{ampm}');

  Date.prototype.__defineGetter__('hh12', function() {
    var hour = this.hour();
    var ampm = hour < 12 ? 'AM' : 'PM';
    return HH12({
      hour: (hour % 12 || 12),
      minute: pad_number(this.minute()),
      ampm: hour < 12 ? 'AM' : 'PM'
    });
  });

  Date.prototype.__defineGetter__('hh24', function() {
    return HH24({
      hour: pad_number(this.hour()),
      minute: pad_number(this.minute())
    });
  });
